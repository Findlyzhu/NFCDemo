...
public class SKFType {
    ...
    **/
    public class ECCPUBLICKEYBLOB{
        ...
        public void     set_pubkey(byte[] pdata){
            this.BitLen = DCUtil.bytesToIntS(pdata,0);
            System.arraycopy(pdata,4,this.XCoordinate,0,ECC_MAX_XCOORDINATE_BITS_LEN/8);
            System.arraycopy(pdata,4+ECC_MAX_XCOORDINATE_BITS_LEN/8,this.YCoordinate,0,ECC_MAX_XCOORDINATE_BITS_LEN/8);
        }
    }
    ...
    public class ENVELOPEDKEYBLOB{
        ...
        public ECCPUBLICKEYBLOB PubKey;        // 加密密钥对的公钥
        public ECCCIPHERBLOB ECCCipherBlob;    // 用保护公钥加密的对称密钥密文。
        public byte[] get_pubkey_x(){
            return this.PubKey.XCoordinate;
        }
        public byte[] get_pubkey_y(){
            return this.PubKey.YCoordinate;
        }
        public int get_pubkey_bit(){
            return this.PubKey.BitLen;
        }
        public byte[] get_cipher_x(){
            return this.ECCCipherBlob.XCoordinate;
        }
        public byte[] get_cipher_y(){
            return this.ECCCipherBlob.YCoordinate;
        }
        public byte[] get_cipher_hash(){
            return this.ECCCipherBlob.HASH;
        }
        public byte[] get_cipher_cipher(){
            return this.ECCCipherBlob.Cipher;
        }
        public int get_cipher_cipherlen(){
            return this.ECCCipherBlob.CipherLen;
        }
        public void set_pubkey(int BitLen,byte[] XCoordinate,byte[] YCoordinate){
            this.PubKey.BitLen = BitLen;
            System.arraycopy(XCoordinate,0,this.PubKey.XCoordinate,0,ECC_MAX_XCOORDINATE_BITS_LEN/8);
            System.arraycopy(YCoordinate,0,this.PubKey.YCoordinate,0,ECC_MAX_XCOORDINATE_BITS_LEN/8);
            //this.PubKey.XCoordinate = XCoordinate;
            //this.PubKey.YCoordinate = YCoordinate;
        }
        public void set_cipher(byte[] XCoordinate,byte[] YCoordinate,byte[] HASH,int CipherLen,byte[] Cipher){
            this.ECCCipherBlob.CipherLen = CipherLen;
            System.arraycopy(XCoordinate,0,this.ECCCipherBlob.XCoordinate,0,ECC_MAX_XCOORDINATE_BITS_LEN/8);
            System.arraycopy(YCoordinate,0,this.ECCCipherBlob.YCoordinate,0,ECC_MAX_XCOORDINATE_BITS_LEN/8);
            System.arraycopy(HASH,0,this.ECCCipherBlob.HASH,0,32);
            System.arraycopy(Cipher,0,this.ECCCipherBlob.Cipher,0,1);
            //this.ECCCipherBlob.XCoordinate = XCoordinate;
            //this.ECCCipherBlob.YCoordinate = YCoordinate;
            //this.ECCCipherBlob.HASH = HASH;
            //this.ECCCipherBlob.Cipher = Cipher;
        }
    }
